...
public class ClosestStationFinder {
    private final GeoCalculations geoCalc = new GeoCalculations();

    public StationInformation.Station findClosestStationWithBikes(double userLat, double userLon,
        ...
        return stationInfo.data.stations.stream()
                ...
                //the filter is filtering through the stations assessing the station status
                .min(Comparator.comparingDouble(station ->
                        geoCalc.haversine(userLat, userLon, station.lat, station.lon)))
    ...
    public StationInformation.Station findClosestStationWithDocks(double userLat, double userLon,
                                                                  StationInformation stationInfo,
                                                                  StationStatus stationStatus) {
        return stationInfo.data.stations.stream()
